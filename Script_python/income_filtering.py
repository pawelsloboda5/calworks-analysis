...
def calculate_income_metrics(config: Config) -> pd.DataFrame:
    ...
    all_persons[income_cols] = (
        all_persons[income_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
    )
    ...
    updated_households = pd.merge(
        eligible_households, household_income_aggregates, on="SERIALNO", how="left"
    )
