...
def load_pums_data(household_data_path: str, person_data_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        logger.info(f"Initial data shapes - Households: {household_df.shape}, Persons: {person_df.shape}")
        ...
        household_df = household_df[
            (household_df['ST'] == 6) & 
            (household_df['PUMA'].isin(sf_puma_codes))
        ...
        logger.info(f"Successfully loaded {len(household_df)} households and {len(person_df)} persons")
...
def aggregate_person_data(person_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        aggregated_person_df = person_df.groupby('SERIALNO').agg({
            'PAP': ['sum', lambda x: (x > 0).any()]
        }).reset_index()
...
def calculate_eligibility(household_df: pd.DataFrame, aggregated_person_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        household_df['MBSAC'] = household_df['NP_capped'].map(config['mbsac_thresholds'])
