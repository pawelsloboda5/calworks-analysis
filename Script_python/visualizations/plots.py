...
def plot_income_analysis(df: pd.DataFrame, output_dir: Union[str, Path]) -> None:
    ...
    source_presence = pd.DataFrame({
        'PUMA': df['PUMA'].astype(str),
        'Employment': df['households_with_employment_income'] / df['total_households'] * 100,
        'Public Assistance': df['households_with_public_assistance_income'] / df['total_households'] * 100,
        'Retirement': df['households_with_retirement_income'] / df['total_households'] * 100,
        'Dividends': df['households_with_dividend_income'] / df['total_households'] * 100,
        'Social Security': df['households_with_social_security_income'] / df['total_households'] * 100
    })
    ...
    sns.barplot(x='PUMA', y='Percentage', hue='Income Source', data=source_presence_melted, ax=ax4)
