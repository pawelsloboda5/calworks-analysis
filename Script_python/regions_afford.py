...
def analyze_regions(eligible_households, eligible_persons):
    ...
    eligible_households['has_employment_income'] = eligible_households['total_employment_income'] > 0
    eligible_households['has_public_assistance_income'] = eligible_households['total_public_assistance_income'] > 0
    eligible_households['has_retirement_income'] = eligible_households['total_retirement_income'] > 0
    eligible_households['has_dividend_income'] = eligible_households['total_dividend_income'] > 0
    eligible_households['has_social_security_income'] = eligible_households['total_social_security_income'] > 0

    # Aggregate households by region
    household_summary = eligible_households.groupby('PUMA').agg(
        total_households=('SERIALNO', 'count'),
        median_rent=('GRNTP', 'median'),
        median_income=('monthly_income', 'median'),
        median_income_to_rent_ratio=('monthly_income', lambda x: x.median() / eligible_households.loc[x.index, 'GRNTP'].median()),
        median_employment_income=('total_employment_income', 'median'),
        median_public_assistance_income=('total_public_assistance_income', 'median'),
        ...
        households_with_public_assistance_income=('has_public_assistance_income', 'sum'),
        households_with_retirement_income=('has_retirement_income', 'sum'),
        households_with_dividend_income=('has_dividend_income', 'sum'),
        households_with_social_security_income=('has_social_security_income', 'sum')
    ).reset_index()
    ...
    person_summary = eligible_persons.groupby('PUMA').agg(
        total_people=('SERIALNO', 'count')
    ).reset_index()
